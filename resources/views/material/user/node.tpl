						...
						{if $node['class']!=$class}
							...
							<div class="nodetitle">
								<a class="waves-effect waves-button" data-toggle="collapse" href="#cardgroup{$class}" aria-expanded="true" aria-controls="cardgroup{$class}">
								    <span>{if $class == 0}普通{/if}{if $class < 22}VIP{/if}{if $class >=23}SVIP{/if}用户节点</span><i class="material-icons">expand_more</i>
								</a>
							</div>
						...
						<div class="node-card node-flex" cardindex="{$node@index}">
                            <div class="nodemain">
                                ...
                                <div class="nodemiddle node-flex">
                                    <div class="onlinemember node-flex"><i class="material-icons node-icon">flight_takeoff</i><span>节点实时 {if $node['online_user'] == -1} N/A{else} {$node['online_user']}{/if}</span> 人在线</div>
                                    <!-- <div class="nodetype">{$node['status']}</div> -->
                                </div>
                                <div class="nodeinfo node-flex">
                                    <div class="nodetraffic node-flex"><i class="material-icons node-icon">equalizer</i><span>节点累计已使用 {if $node['traffic_limit']>0}{$node['traffic_used']}/{$node['traffic_limit']}GB{else}{$node['traffic_used']}GB{/if}</span> 流量</div>
