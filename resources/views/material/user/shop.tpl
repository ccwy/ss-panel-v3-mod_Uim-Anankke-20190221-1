						...
						<div class="card-main">
							<div class="card-inner">
								<p><i class="icon icon-lg">monetization_on</i>&nbsp;余额：&nbsp;<font color="red" size="5">{$user->money}</font>&nbsp;元</p>
								    ...
								    3，<font color="red">套餐不可叠加，新购套餐会覆盖旧套餐的效果；</font><br>
									4，<font color="red">不同套餐可用节点数量不同，高等级可使用全部低等级节点，购买前请先</font><a href="/user/node">点击这里查看节点列表</a><br>
								    5，本站不提供退款，本站不提供任何技术支持，使用方法请看<a href="/user/guide">使用教程</a>；<br>
                  ...
                  <div class="card">
					  <div class="card-main">
								...
								<div class="shop-cube">
									...
									<div>
										<div class="cube-detail">
											{if $shop->id == 60 }{$shop->connector()} <span>个</span>{elseif $shop->id == 76}30 <span>个</span>{elseif $shop->id == 68}不变{else}3 <span>个</span>{/if}
										</div>
										<div class="cube-title">
											客户端数量
										</div>
					...
					<div class="shop-gridarea">
						...
						{if $shop->id != 60 && $shop->id != 68}						       					       
							...
							{/if}
								...
								<div class="card-tag tag-black">客户端限制</div> <div class="card-tag tag-blue">{if $shop->id == 60 }{$shop->connector()} 个{elseif $shop->id == 68}无{elseif $shop->id == 76}30 个{else}3 个{/if}</div>
						{/if}		
						{if $shop->id == 68 }
