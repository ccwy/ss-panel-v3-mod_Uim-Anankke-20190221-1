											...
											var chart = new CanvasJS.Chart("alive_chart",
											{
												...
												data: [
												{
													...
													dataPoints: [
														...
														{
															y: {$sts->lastMonthIncome()/$sts->thisyearIncome()*100}, legendText:"{date("Y-m%"), strtotime("-1 months")}月流水 {number_format($sts->lastMonthIncome()/$sts->thisyearIncome()*100,2)}% {$sts->lastMonthIncome()}元", indexLabel: "{date("m")-1}月流水 {number_format($sts->lastMonthIncome()/$sts->thisyearIncome()*100,2)}% {$sts->lastMonthIncome()}元"
														},
														{
