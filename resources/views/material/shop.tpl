					...
					<div class="card">
						<div class="card-main">
							<div class="card-inner">
								   ...
								   <p> 
								     1，本站无游戏节点，不保证可以观看TVB\NF\huhu等视频网站；<br>
								     2，套餐不可叠加，新购套餐会覆盖旧套餐的效果，本站不提供任何技术支持，使用方法请看教程；<br>
									 3，注册充值购买即代表您同意并愿意遵守本站<a href="/toos">用户协议（TOS）</a>。
								   </p>
							</div>
						</div>
					</div>
                  ...
                  <div class="card">
					  <div class="card-main">
								...
								<div class="shop-cube">
									...
									<div>
										<div class="cube-detail">
											{if $shop->id == 60 }{$shop->connector()} <span>个</span>{elseif $shop->id == 68}不变{else}3 <span>个</span>{/if}
										</div>
										<div class="cube-title">
											客户端数量
										</div>
					...
					<div class="shop-gridarea">
						...
						{if $shop->id != 60 && $shop->id != 68}						       					       
							...
							{/if}
								...
								<div class="card-tag tag-black">客户端限制</div> <div class="card-tag tag-blue">{if $shop->id == 60 }{$shop->connector()} 个{elseif $shop->id == 68}无{else}3 个{/if}</div>
						{/if}		
						{if $shop->id == 68 }
