...
class UserController extends BaseController
{
    ...
    {
        $user = Auth::getUser();
        $nodes = Node::where('type', 1)->orderBy('node_class')->orderBy('name', 'desc')->get();
    ...
    public function announcement($request, $response, $args)
    {
       ...
       // return $this->view()->assign("anns", $Anns)->display('user/announcement.tpl');		
        ...
        $Anns = Ann::orderBy('date', 'desc')->paginate(10, ['*'], 'page', $pageNum);
...
//连接密码不能小于8位
    ...
    {    
		...
		$time =  date('YmdHis');
		if ($this->user->t != 0 && strlen($code_olrid) >= 28) {
        ...
        $this->user->save();  
        $res['ret'] = 1;
        $res['msg'] = "工单提交成功，本次已自动处理完成，您本次充值的 ".$code_money." 元已到账，您可以返回充值页面查看余额；<br>请注意：本次是系统为您自动补单，本次自动补单结果需要管理员复核，管理员复核后会回复工单处理结果。<br>本次已提交成功，请勿重复提交工单。";
		} else {		
		$res['ret'] = 1;
        $res['msg'] = "工单提交成功，请等待管理员处理，工单有回复就处理了，请留意工单回复内容。<br>本次已提交成功，请勿重复提交工单。";
		}
        return $this->echoJson($response, $res);
