...
class CodeController extends AdminController
{
    ...
    public function ajax_code($request, $response, $args)
    {
        ...
        $datatables->query('Select code.id,code.code,code.type,code.number,code.isused,code.userid as user.id,user.user_name as user_name,code.usedatetime from code,user where code.userid = user.id');
