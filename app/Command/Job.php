...
class Job
{
    ...
    public static function DailyJob()
    {
        ...
        Speedtest::where("datetime", "<", time()-3600)->delete();
    ...
    public static function CheckJob()
    {
		...
		$subject = Config::get('appName')."-您的账户已经过期了";
            ...
            if (Config::get('account_expire_delete_days')>=0&&
				strtotime($user->expire_in)+Config::get('account_expire_delete_days')*86400<time() &&  $user->class == 0
			) {
                $subject = Config::get('appName')."-您的账号已经被删除了";
                $to = $user->email;
                $text = "您好，感谢您的支持与使用，系统发现您的账号等级已于 ".$user->class_expire." 过期，根据网站用户协议约定，账号过期  7   天以上会被系统自动删除，您的帐号现已被删除，如需继续使用，请重新注册，如有疑问，请发邮件联系管理员，Email：".Config::get('cousemail');
                try {
                    Mail::send($to, $subject, 'news/warn.tpl', [
                        "user" => $user,"text" => $text
                    ], [
 ...
 //删除注册后3天未购买账户
		...
		/*	if($user->class ==0 && time() - strtotime($user->class_expire) >= 7*86400 && $user->tixing ==3)
            ...
            if ($user->class!=0 && 
                ...
                $subject = Config::get('appName')."-您的账号等级已经到期了";
		$text = "您好，系统发现您的账号等级已经到期，账号已被暂停使用，账号过期  7  天以上将自动删除，为不影响您的正常使用，请及时登陆网站到商店购买套餐重新激活账号，如有疑问，请登陆网站发工单联系管理员，谢谢！" ;
                $to = $user->email;              
                try {
                    Mail::send($to, $subject, 'news/warn.tpl', [
                        "user" => $user,"text" => $text
