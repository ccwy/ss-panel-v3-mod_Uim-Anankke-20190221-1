...
class AopF2F extends AbstractPayment
{
    private function createGateway(){
        ...
        $gateway->setNotifyUrl(Config::get("payurl")."/payment/notify");
    ...
    function purchase($request, $response, $args)
    {
        ...
        $request->setBizContent([
            'subject'      => "￥".$pl->total." - ".Config::get("payname")." - {$user->id}({$user->port})",
    ...
    function notify($request, $response, $args)
    {
        ...
        try {
			...
			$trade_no = $aliResponse->data('trade_no'); //支付宝订单号
            if($aliResponse->isPaid()){
                self::postPayment($pid, $trade_no);
